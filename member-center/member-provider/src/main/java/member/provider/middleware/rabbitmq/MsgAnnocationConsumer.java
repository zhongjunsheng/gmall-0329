...
//    //fanout 广播 类型
//    @RabbitListener(bindings = @QueueBinding(
//            value = @Queue(value ="fan_queue2",durable = "true"),  //表示队列信息
//            exchange = @Exchange(value = "fanout_exchange",durable = "ture",//绑定的交换机名称
//                    type = ExchangeTypes.FANOUT,//交换机类型 direct，topic，fanout
//                    ignoreDeclarationExceptions = "true") //支持过滤
//    ))
//    //@RabbitHandler
//    public void  getfanoutMsg2(Message message, Channel changel) throws Exception{
//        System.out.println("收到的信息是:"+ message.getPayload() +"消费者2");
//        //消息消费ack -- 表示已经消费了
//        Long tag = (Long)message.getHeaders().get(AmqpHeaders.DELIVERY_TAG);
//        //告诉服务器说明这条消息以及被消费了，否则服务器会以为消息没有被消费 会继续发送
//        changel.basicAck(tag,false); //false 表示手动ack
...
//
//    @RabbitListener(bindings = @QueueBinding(
//            value = @Queue(value ="queue2",durable = "true"),  //表示队列信息
//            exchange = @Exchange(value = "exchange",durable = "ture",//绑定的交换机名称
//                    type = ExchangeTypes.TOPIC,//交换机类型 direct，topic，fanout
//                    ignoreDeclarationExceptions = "true"),  //支持过滤
//            key = "allen.*"  //表示路由信息
//    ))
//    public void  getMsg2(Map<String, Object> map,Message message, Channel changel) throws Exception{
//        System.out.println("用户2收到的信息是:"+ map.get("msg"));
//        //消息消费ack -- 表示已经消费了
//        Long tag = (Long)message.getHeaders().get(AmqpHeaders.DELIVERY_TAG);
//        changel.basicAck(tag,false); //false 表示手动ack
//    }
